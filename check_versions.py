...
def extract_github_repos_from_readme(readme_path: str) -> List[Tuple[str, str]]:
    ...
    try:
        ...
        for name, url in matches:
            ...
            if any(
                skip in url
                for skip in ["/releases", "/actions", "/badge.svg", ".png", ".jpg"]
            ):
                continue
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if version_match:
                    ...
                    published = (
                        date_match.group(1).split("T")[0] if date_match else "N/A"
                    )
...
def main():
    ...
    for name, url in repos:
        ...
        if "wav-files-" in url or (
            "RustedBytes" in url and "wav-files-toolkit" not in url
        ):
